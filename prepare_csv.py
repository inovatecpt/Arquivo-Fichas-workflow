...
from get_max_id import get_highest_ids
...
def main(input_path, collection_id, output_path=None):
    ...
    create_csv(group_csv_path, ["id", "name", "group_type", "collection_id"])
    create_csv(record_csv_path, ["id", "group_id", "string"])
    create_csv(image_csv_path, ["id", "file", "text", "record_id", "search_vector", "type"])
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("Usage: python script.py /path/to/main_dir <collection_id> [optional:/path/to/output_dir]")
        sys.exit(1)
